...
//Builds HTML for single category page based on the 
//data from the server
function buidAndShowMenuItemsHTML (categoryMenuItems) {
	//Load title snnipet of menu items page
	 $ajaxUtils.sendGetRequest(
	 	menuItemsTitleHtml,
	 		...
	 		$ajaxUtils.sendGetRequest(
	 			...
	 			function (menuItemHtml) {
	 				...
	 				var menuItemsViewHtml = 
	 					builMenuItemsViewHtml(categoryMenuItems,
	 									      menuItemsTitleHtml,
	 										  menuItemHtml);
...
//Using category and menu itmes data and snippets html
//build menu items view HTML to be inserted into page
function builMenuItemsViewHtml (categoryMenuItems,
	 						    menuItemsTitleHtml,
	 						    menuItemHtml) {
	 menuItemsTitleHtml =
	 	...
	 	for (var i =0; i < menuItems.length; i++) {
	 		...
	 		html =
	 			insertProperty(html, "catShortName", 
	 								  catShortName);
	 		html =
	 			insertItemPrice(html, "price_small", 
	 								  menuItems[i].price_small);
	 		html =
	 			insertItemPortionName(html, "small_portion_name", 
	 							      menuItems[i].small_portion_name);
	 		html =
	 			insertItemPrice(html, "price_large", 
	 							      menuItems[i].price_large);
	 		html =
	 			insertItemPortionName(html, "large_portion_name", 
	 							      menuItems[i].large_portion_name);
	 		html =
	 			insertProperty(html, "name", 
	 								  menuItems[i].name);
	 		html =
	 			insertProperty(html, "description", 
	 								  menuItems[i].description);

	 		//Add clearfix after every second menu item
	 		if (i % 2 != 0) { //every odd number loop starts 0
	 			html += 
